...
## Suggested packaging mapping

For installable distribution, package by contract boundary:

- `rubber_duck/router/` → routing rules and preflight policy
- `rubber_duck/agents/` → duckling prompt contracts
- `rubber_duck/skills/` → behavior playbooks
- `rubber_duck/templates/` → config + setup templates

This preserves architecture intent as package structure.
